...
def neighbors(point, board)
  ...
  allowed_directions = if point.directions.count >= 10 && point.directions.last(10).uniq.count == 1
                       ...
                       elsif point.directions.count == 0
                         [UP, RIGHT, DOWN, LEFT]
                       elsif point.directions.count < 4
                         [point.directions.last]
                       elsif point.directions.last(4).uniq.count != 1
                         [point.directions.last]
                       else
                         [UP, RIGHT, DOWN, LEFT]
                       end
...
def find_minimal_path(start_pos, final_pos, board)
  ...
  while current_point = queue.pop  do
    if current_point.pos == final_pos
      if current_point.directions.last(4).uniq.count == 1
    ...
    key = [current_point.pos, current_point.directions.last(10)]
...
p task2: find_minimal_path(start_pos, final_pos, BOARD)
